...
class ResourceQualityGate:
    ...
    def __init__(self) -> None:
        """初始化 ResourceQualityGate 所需的依赖。"""
        self.defaults = ResourceDefaultFactory()

    def normalize(
        self, resource: dict[str, Any], task_title: str, next_action: str
    ) -> dict[str, Any]:
        """校验、修复并规范化结构化学习资源。"""
        ...
        description = str(
            resource.get("description") or self.description(type_, next_action)
        )
        ...
        content = str(
            resource.get("content") or self.markdown_from_detail(type_, detail, focus)
        )
        return {
            "type": type_,
            "title": title,
            ...
            "recommendation_reason": str(
                resource.get("recommendation_reason")
                or f"当前目标是“{next_action}”，该资源可直接支持本阶段学习。"
            ),
        }
    ...
    def _repair_detail(
        self, type_: str, detail: dict[str, Any], task_title: str, next_action: str
    ) -> dict[str, Any]:
        """修复并补全资源详情结构。"""
                ...
                repaired.append(
                    {
                        ...
                        "type": q_type
                        or ("single_choice" if options else "short_answer"),
                        "stem": str(stem),
                        "options": options,
                        "answer": answer,
                        "analysis": str(
                            ...
                            or base.get("analysis", "请复盘关键步骤。")
                        ),
                        "difficulty": str(
                            question.get("difficulty") or base.get("difficulty", "基础")
                        ),
                        "knowledge_points": question.get("knowledge_points")
                        or base.get("knowledge_points", []),
                    }
                )
            detail["questions"] = repaired
        if type_ == RESOURCE_TYPE_LECTURE:
            detail = self._repair_lecture_detail(
                detail, fallback, task_title, next_action
            )
        if type_ in {RESOURCE_TYPE_MINDMAP, RESOURCE_TYPE_KG}:
            detail["nodes"] = self._repair_nodes(
                detail.get("nodes", []), fallback["nodes"]
            )
            detail["edges"] = self._repair_edges(
                detail.get("edges", []), fallback["edges"]
            )
    ...
    def _repair_lecture_detail(
        ...
        for index, section in enumerate(sections):
            ...
            heading = str(
                section.get("heading") or section.get("title") or base["heading"]
            )
            body = str(
                section.get("body")
            ...
            repaired_sections.append(
                {
                    ...
                    "overview": str(
                        ...
                        or self._first_sentence(body)
                        or base["body"]
                    ),
                    "steps": self._repair_lecture_steps(
                        section.get("steps"), body, heading, next_action
                    ),
                    "key_points": self._repair_text_list(
                        section.get("key_points") or section.get("keyPoints"),
                        self._derive_key_points(body, heading),
                    ),
                    "common_mistakes": self._repair_text_list(
                        section.get("common_mistakes")
                        or section.get("mistakes")
                        or section.get("warnings"),
                        self._derive_mistakes(body, heading),
                    ),
                    "example": str(
                        section.get("example")
                        or self._derive_example(body, heading, task_title)
                    ),
                    "self_check": str(
                        section.get("self_check")
                        or section.get("selfCheck")
                        or self._derive_self_check(heading)
                    ),
                }
            )
        detail["sections"] = repaired_sections or fallback["sections"]
        detail["key_points"] = self._repair_text_list(
            detail.get("key_points"),
            fallback.get("key_points", [task_title, next_action]),
        )
        detail["checkpoints"] = self._repair_text_list(
            detail.get("checkpoints"), fallback.get("checkpoints", [])
        )
        return detail

    def _repair_lecture_steps(
        self, steps: Any, body: str, heading: str, next_action: str
    ) -> list[str]:
        """补全讲解文档中缺失的学习步骤。"""
        existing = [
            item
            for item in self._repair_text_list(steps, [])
            if not any(
                marker in item
                for marker in [
                    "涉及的关键对象",
        ...
        for item in [
            *existing,
            *extracted,
            *self._derive_steps(body, heading, next_action),
        ]:
            text = str(item).strip(" ；;。")
    ...
    def _derive_steps(body: str, heading: str, next_action: str) -> list[str]:
        ...
        example_sentence = next(
            (
                ...
                if any(word in item for word in ["例如", "例子", "比如"])
            ),
            "",
        )
        return [
            ...
            example_sentence
            or f"然后选一个与“{heading}”相关的小例子，按本节规则完整演示一遍。",
    ...
    def _derive_key_points(body: str, heading: str) -> list[str]:
        ...
        sentences = [
            item.strip() for item in re.split(r"[。！？；;]", body) if item.strip()
        ]
        matched = [
            item
            for item in sentences
            if any(
                word in item
                for word in ["核心", "概念", "关键", "规则", "方法", "条件", "例"]
            )
        ]
        if matched:
            return matched[:4]
    ...
    def _derive_mistakes(body: str, heading: str) -> list[str]:
        ...
        matched = [
            ...
            if any(
                word in item
                for word in ["易错", "错误", "注意", "不能", "不要", "混淆"]
            )
        ]
        if matched:
            return matched[:3]
        return [
            f"学习“{heading}”时容易只记结论，忽略条件和步骤。",
            "做题后只看答案，不复盘错误发生在哪一步。",
        ]
    ...
    def _options_are_only_labels(options: Any) -> bool:
        ...
        if not isinstance(options, list) or not options:
            return True
        return all(
            bool(re.fullmatch(r"[A-Da-d]", str(item).strip())) for item in options
        )
    ...
    def _extract_inline_options(stem: str) -> tuple[str, list[str]]:
        ...
        matches = re.findall(
            r"([A-Da-d])[.、]\s*(.*?)(?=\s+[A-Da-d][.、]\s*|$)", options_text
        )
    ...
    @staticmethod
    def _repair_nodes(
        nodes: Any, fallback: list[dict[str, Any]]
    ) -> list[dict[str, Any]]:
        """清洗并补全图谱节点。"""
            ...
            repaired.append(
                {
                    ...
                    **{
                        ...
                        if key not in {"id", "label", "name", "title", "text"}
                    },
                }
            )
    ...
    @staticmethod
    def _repair_edges(
        edges: Any, fallback: list[dict[str, Any]]
    ) -> list[dict[str, Any]]:
        """清洗并补全图谱关系。"""
    ...
    @staticmethod
    def markdown_from_detail(
        type_: str, detail: dict[str, Any], task_title: str
    ) -> str:
        """将结构化资源详情转换为 Markdown。"""
        if type_ == RESOURCE_TYPE_LECTURE:
            sections = detail.get("sections", [])
            blocks = []
        ...
        if type_ == RESOURCE_TYPE_EXERCISE:
            return "\n\n".join(
                f"{i + 1}. {q.get('stem')}"
                for i, q in enumerate(detail.get("questions", []))
            )
        if type_ in {RESOURCE_TYPE_MINDMAP, RESOURCE_TYPE_KG}:
