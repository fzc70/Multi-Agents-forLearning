...
PROFILE_RULES = (
    DimensionRule("weakness", (r"不会|不懂|困难|卡住|出错|错题|薄弱|混淆|看不懂",), "来自对话中的困难表达"),
    DimensionRule("goal", (r"目标|希望|想要|准备|提升|掌握|通过|学会",), "来自对话中的目标表达"),
    DimensionRule("preference", (r"例子|代码|视频|图|练习|文档|案例|动画|实操",), "来自对话中的资源偏好"),
    DimensionRule("foundation", (r"基础|学过|没学过|入门|零基础|复习|了解过",), "来自对话中的基础描述"),
)

INTENT_RULES = (
    IntentRule("generate_resource", (r"生成|资料|资源|练习|题|文档|导图|图谱|阅读|案例",)),
    IntentRule("plan", (r"路径|计划|下一步|安排|顺序|怎么学",)),
    IntentRule("assessment", (r"测评|测试|掌握|评分|评估|检测",)),
    IntentRule("profile_update", (r"我的基础|我的目标|我喜欢|我希望|我想要",), need_retrieval=False),
)

EMPTY_MEANING_WORDS = {"无", "暂无", "没有", "无明显", "暂无明显", "none", "null", "n/a"}

KG_STOP_WORDS = {"我们", "可以", "当前", "学习", "资料", "任务", "这个", "一个", "进行", "完成"}
...
def infer_intent(text: str) -> dict[str, Any]:
    for rule in INTENT_RULES:
        if any(re.search(pattern, text, re.I) for pattern in rule.patterns):
            return {"intent": rule.intent, "need_retrieval": rule.need_retrieval, "confidence": 66}
...
def clean_meaningful_values(values: Any) -> list[str]:
    ...
    for value in values:
        ...
        if not text or text.lower() in EMPTY_MEANING_WORDS or text.startswith(("无明显", "暂无明显")):
            continue
        cleaned.append(text)
